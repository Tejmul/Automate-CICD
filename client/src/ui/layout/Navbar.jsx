...
import { useCart, useCartWithProducts, useWishlist, useCartActions } from '../hooks/useStorefront'
      ...
      <header className={`navbar ${scrolled ? 'navbar--scrolled' : ''}`}>
        <div className="navbar__inner container">
          <Link to="/" className="navbar__logo">
            <div className="navbar__logo-mark">
              <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
                <path d="M10 2L17.3 6.25V13.75L10 18L2.7 13.75V6.25L10 2Z" fill="currentColor" />
              </svg>
            </div>
          ...
          <nav className="navbar__nav">
            ...
            <Link to="/shop" className={`navbar__link ${isActive('/shop') ? 'active' : ''}`}>Shop</Link>
            <Link to="/wishlist" className={`navbar__link ${isActive('/wishlist') ? 'active' : ''}`}>
              Wishlist
              {wishlistCount > 0 && <span className="navbar__badge">{wishlistCount}</span>}
            </Link>
            <Link to="/orders" className={`navbar__link ${isActive('/orders') ? 'active' : ''}`}>Orders</Link>
          </nav>
