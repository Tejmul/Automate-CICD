...
describe("HomePage", async () => {
  const { useProducts, useCartActions, useWishlist, useWishlistActions } =
    await import("../hooks/useStorefront");
  ...
  it("shows featured skeletons while loading", () => {
    ...
    useWishlistActions.mockReturnValue({
      add: { mutate: vi.fn() },
      remove: { mutate: vi.fn() },
    });
  ...
  it("renders product cards and wires quick-add + wishlist toggle", () => {
    ...
    useProducts.mockReturnValue({
      data: { products: [productA, productB] },
      isLoading: false,
    });
    ...
    expect(screen.getByRole("link", { name: /alpha tee/i })).toHaveAttribute(
      "href",
      `/product/${productA.id}`,
    );
    expect(screen.getByRole("link", { name: /beta hat/i })).toHaveAttribute(
      "href",
      `/product/${productB.id}`,
    );
