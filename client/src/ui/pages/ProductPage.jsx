...
export function ProductPage() {
  ...
  const wishlistSet = useMemo(
    () => new Set(wishlist.map((x) => x.productId)),
    [wishlist],
  );
  ...
  return (
    <div className="page pdp">
      ...
      {isLoading || !product ? (
        ...
        <div className="pdp__grid">
          <div className="pdp__media">
            <img className="pdp__img" src={currentImg} alt={product.title} />
            {images.length > 1 && (
          ...
          <div className="pdp__info">
            ...
            <div className="pdp__meta">
              ...
              <span
                className={`pdp__stock ${product.stock > 0 ? "pdp__stock--in" : "pdp__stock--out"}`}
              >
                {product.stock > 0
            ...
            {tags.length > 0 && (
              <div className="pdp__tags">
                {tags.map((tag) => (
                  <span key={tag} className="pdp__tag">
                    #{tag}
                  </span>
                ))}
              </div>
            ...
            <div className="pdp__actions">
              <button
                ...
                onClick={() =>
                  cartActions.add.mutate({ productId: product.id, quantity: 1 })
                }
              >
                {cartActions.add.isPending
                  ? "Adding…"
                  : product.stock === 0
                    ? "Out of Stock"
                    : "Add to Cart"}
              ...
              <button
                ...
                type="button"
                disabled={
                  wishActions.add.isPending || wishActions.remove.isPending
                }
                onClick={() => {
                  if (wishlistSet.has(product.id))
                    wishActions.remove.mutate(product.id);
            ...
            {(cartActions.add.error ||
              wishActions.add.error ||
              wishActions.remove.error) && (
              <InlineError
                title="Action failed"
                message={
