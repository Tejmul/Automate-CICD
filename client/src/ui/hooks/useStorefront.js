...
export function useProducts({ q, category, limit = 24, skip = 0 } = {}) {
    return useQuery({
        queryKey: ['products', { q: q || null, category: category || null, limit, skip }],
        queryFn: () =>
            category
...
export function useWishlistWithProducts() {
    ...
    const isLoading = wishlist.isLoading || productQueries.some((q) => q.isLoading)
...
export function useCartActions() {
    ...
    const add = useMutation({
        mutationFn: ({ productId, quantity }) => api.cart.add({ productId, quantity }),
        onSuccess: invalidate,
    })
    const setQuantity = useMutation({
        mutationFn: ({ productId, quantity }) => api.cart.setQuantity({ productId, quantity }),
        onSuccess: invalidate,
    })
