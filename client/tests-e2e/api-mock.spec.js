...
test.describe('Mocked API E2E', () => {
    test('mocked GET /api/products returns JSON and page loads', async ({ page }) => {
        await page.route('**/api/products**', (route) => {
            route.fulfill({
                ...
                body: JSON.stringify({
                    products: [
                        { id: 1, title: 'Mock T-Shirt', price: 29.99, thumbnail: '', images: [], tags: [], category: 'clothing', stock: 10, rating: 4.5, brand: 'TestBrand' },
                        { id: 2, title: 'Mock Jeans', price: 59.99, thumbnail: '', images: [], tags: [], category: 'clothing', stock: 5, rating: 4.0, brand: 'TestBrand' },
                    ],
                    total: 2,
                }),
            })
    ...
    test('mocked GET /api/products/categories returns categories', async ({ page }) => {
        await page.route('**/api/products/categories', (route) => {
            route.fulfill({
                status: 200,
                contentType: 'application/json',
                body: JSON.stringify({ categories: ['beauty', 'furniture', 'clothing'] }),
            })
        })
    ...
    test('mocked product detail page renders', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: /premium sneakers/i })).toBeVisible()
